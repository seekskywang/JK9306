...
/*
void LCD_StrClear( uint16_t x0, uint16_t y0, uint8_t len , uint32_t color )
{
		LCD_DrawRect(x0,y0,x0+10*len,y0,color);
}
*/
...
* Attention		 : None
*******************************************************************************/
void LCD_DrawLineShade( uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1 , uint8_t red,uint8_t green,uint8_t blue,uint8_t big_small,uint8_t colorflag )
{
  short dx,dy;
  short temp;
  uint32_t color;
...
* Attention		 : None
*******************************************************************************/
void PutChar( uint16_t X,uint16_t Y,uint8_t ASCI,uint32_t charColor,uint32_t bkColor )
{
	uint32_t i, j;
        ...
        for( j=0; j<8; j++ )
        {
            if (tmp_char >> (7 - j) & 0x01 ) //)== 0x01
               ...
               LCD_SetPoint( X + j, Y + i, bkColor );
				...
				LCD_SetPoint( X - 2, Y + i, bkColor );
				LCD_SetPoint( X - 1, Y + i, bkColor );
				LCD_SetPoint( X + 8, Y + i, bkColor );
				LCD_SetPoint( X + 9, Y + i, bkColor );
    }
}

void PutNum( uint16_t X,uint16_t Y,uint8_t ASCI,uint32_t charColor,uint32_t bkColor )
{
	PutChar(X,Y,ASCI+48,charColor,bkColor );
}

void PutCharShade(uint16_t X,uint16_t Y,uint8_t ASCI,uint32_t charColor,uint8_t red,uint8_t green,uint8_t blue,uint8_t big_small,uint8_t colorflag)
{
	uint32_t i, j,bkColor;
    uint8_t buffer[16], tmp_char;
    GetASCIICode(buffer,ASCI);
    for( i=0; i<16; i++ )
    {
		if(colorflag==1)			red-=big_small;
		else if(colorflag==2)			green-=big_small;
		else if(colorflag==3)			blue-=big_small;
		else if(colorflag==4)		{	red-=big_small;		green-=big_small;		}
		else if(colorflag==5)		{	red-=big_small;		blue-=big_small;		}
		else if(colorflag==6)		{	green-=big_small;	blue-=big_small;		}
		else if(colorflag==7)		{	red-=big_small;		green-=big_small;		blue-=big_small;	}		
		bkColor=RGBCOLOR(red,green,blue);
        tmp_char = buffer[i];
        for(j=0;j<8;j++)
        {
            if( (tmp_char >> (7 - j) & 0x01 )== 0x01 )
...
void PutChar_12(uint16_t X,uint16_t Y,uint8_t ASCI,uint32_t charColor,uint32_t bkColor)
{
	uint32_t i, j;
...
** ?   ?: 2010?12?06?
*****************************************************************************/
unsigned int findHzIndex21(const unsigned char *hz)
{
    uint16_t j;
  //NT_GB16 *ptGb16 = (FNT_GB16 *)GBHZ_16;
    for(j=0;j<sizeof(GB_21x20)/sizeof(GB_21x20[0]);j++)
	{
        //if ((*hz == ptGb16->Index[0]) && (*(hz+1) == ptGb16->Index[1])) FNT_GB16//&&(*(hz+1) == GB_21[j].Index[1])
		if((GB_21x20[j].Index[0]==*hz)&&(*(hz+1)==GB_21x20[j].Index[1]))
            return j;
    }
    return 0;
}
...
unsigned int findHzIndex_16(const  char *hz)                                                                       
{//unsigned
    uint16_t j;
  //NT_GB16 *ptGb16 = (FNT_GB16 *)GBHZ_16;
...
void LCD_ShowFontCN_STR_40_55(uint16_t x,uint16_t y,uint16_t xsize,uint16_t ysize,uint8_t * image,uint8_t len)
{
	uint8_t t1;
  uint8_t temp=0;	
	for(t1=0;t1<len;t1++)
	{
	  temp=*(image+t1);       
	 	if(temp>=65&&temp<=90)   //A 0    65 -> 自己的表的33  大写
	  LCD_ShowFontCN_40_55(x+t1*22,y,xsize,ysize,(uint8_t*)nAsciiDot20X40E+102*(temp-65+33));
		else if(temp>=97&&temp<=122)   //A 0    97 -> 自己的表的65  小写
	  LCD_ShowFontCN_40_55(x+t1*22,y,xsize,ysize,(uint8_t*)nAsciiDot20X40E+102*(temp-97+65));
	}
}
void LCD_ShowFontCN_16(uint16_t x,uint16_t y,uint16_t fno,uint8_t mode)
{
	...
	else
	{
		for(t=0;t<16;t++)
		{
			...
			LCD_SetPoint( x - 2, y + t, Colour.black );
			LCD_SetPoint( x - 1 , y + t, Colour.black );
		}
		for(t=0;t<16*2;t++)
		{   
...
void LCD_ShowFontCN21(uint16_t x,uint16_t y,uint16_t fno,uint8_t mode,uint8_t red,uint8_t green,uint8_t blue,uint8_t big_small,uint8_t colorflag)
{                                  
	uint8_t temp,t,t1;
	uint16_t x0=x;
	uint32_t color=RGBCOLOR(red,green,blue);
	temp=0;  
	if(!mode) //正常显示
	{
		for(t=0;t<20*3;t++)
		{   
			temp=GB_21x20[fno].Msk[t];                //gb16Dot[fno][t];    //调用16*08?? 
			...
			for(t1=0;t1<8;t1++)
			{
				if(temp&0x80)
					...
					LCD_SetPoint(x,y,color); 	//Colour.black
				temp<<=1; 
				x++;
				if(x-x0==20)
				{
	...
	else
	{
		for(t=0;t<20*3;t++)
		{   
			temp=GB_21x20[fno].Msk[t];                //gb16Dot[fno][t];    //调用16*08汉字
			for(t1=0;t1<8;t1++)
			{
				if(temp&0x80)
					LCD_SetPoint(x,y,Colour.black);
...
void LCD_ShowFontCN_16_Shade(uint16_t x,uint16_t y,uint16_t fno,uint8_t mode,uint8_t red,uint8_t green,uint8_t blue,uint8_t big_small,uint8_t colorflag)
{                                  
	uint8_t temp,t,t1;
	uint32_t color=RGBCOLOR(red,green,blue);
	uint16_t x0=x;
...
uint16_t WriteOneHzChar21(uint16_t fno,uint16_t x0,uint16_t y0,uint8_t mode,uint8_t red,uint8_t green,uint8_t blue,uint8_t big_small,uint8_t colorflag)
{
	LCD_ShowFontCN21(x0,y0,fno,mode,red,green, blue,big_small,colorflag);
	return (20);                                          /* 返回16位列宽               */
}

uint16_t WriteOneHzChar_16_Shade(uint16_t fno,uint16_t x0,uint16_t y0,uint8_t mode,uint8_t red,uint8_t green,uint8_t blue,uint8_t big_small,uint8_t colorflag)
{
	LCD_ShowFontCN_16_Shade(x0,y0,fno,mode,red,green, blue,big_small,colorflag);
	return (18);                                          /* 返回16位列宽               */
}
...
** ?   ?: 2010?12?06? showstring
*****************************************************************************/
void WriteString21(uint16_t x0,uint16_t y0,const uint8_t *pcStr,uint8_t mode,uint8_t red,uint8_t green,uint8_t blue,uint8_t big_small,uint8_t colorflag)
{
	uint16_t usIndex=0;
	uint16_t usWidth = 0; 
	while(1)
	{
		...
		if(*pcStr > 0x80)                              /* 判断为汉字                  */
		{
			if((x0 + 20) > C_GLCD_H_SIZE)                /* 检查剩余空间是否充足        */
			{
				x0 = 0;
				y0 = y0 + 20;                          /* 改变显示坐标                */
				if(y0 > C_GLCD_V_SIZE)                     /* 纵坐标超出                  */
			...
			usIndex = findHzIndex21(pcStr);
			usWidth = WriteOneHzChar21(usIndex,x0,y0,mode,red,green,blue,big_small,colorflag);
			/*????                     */
			pcStr += 2;
		}
				else
		{
			...
			else 
			{
				...
				PutCharShade( x0, y0+2, *pcStr, Colour.Fword, red, green, blue, big_small,colorflag);   
				/* ASCII码表21H的值对应区位码3区*/
				pcStr += 1;
				usWidth=10;
			}
...
void WriteString_Big(uint16_t x0, uint16_t y0, const char *pcStr)
{
//	uint16_t usIndex=0;
...
void WriteString_16(uint16_t x0, uint16_t y0, const char *pcStr, uint8_t mode)
{
	uint16_t usIndex=0;
	uint16_t usWidth = 0; 
	while(1)
...
//显示16的英文字符渐变
void WriteString_16_Shade(uint16_t x0,uint16_t y0,const char *pcStr,uint8_t mode,uint8_t red,uint8_t green,uint8_t blue,uint8_t big,uint8_t colorflag)
{
	uint16_t usIndex=0;
	uint16_t usWidth = 0;
	while(1)
	{
		if(*pcStr == 0) 
			...
			break;                                     /*字符串结束         */
		x0 = x0 + (usWidth);                           /* 调节字符串的松紧度   */    /* 判断为非汉字                */
		...
		if(*pcStr > 0x80)                              /* 判断为汉字                  */
		{
			...
			usIndex = findHzIndex_16(pcStr);
			usWidth = WriteOneHzChar_16_Shade(usIndex, x0, y0, mode,red, green, blue, big,colorflag);
			/* ????                     */
			pcStr += 2;
		}		
		else
		{
			...
			else 
			{
				...
				//usWidth = WriteOneASCII(pcStr, x0, y0, mode);
				PutCharShade( x0, y0, *pcStr, Colour.Fword, red, green, blue, big,colorflag);   
				/* ASCII码表21H的值对应区位码3区*/
				pcStr += 1;
				usWidth=10;
			}
...
void WriteString_12(uint16_t x0, uint16_t y0, const char *pcStr, uint8_t mode)
{
	uint16_t usIndex=0;
	uint16_t usWidth = 0; 
	while(1)
