...
//	Disp.Voltage = (ComBuf.rec.buf[3]<<8) + ComBuf.rec.buf[4];
	Disp.Voltage[0] = (uint32_t)((double)((ComBuf.rec.buf[3]<<8) + ComBuf.rec.buf[4]) * 4);//µÁ—πA
	Disp.Voltage[1] = (uint32_t)((double)((ComBuf.rec.buf[5]<<8) + ComBuf.rec.buf[6]) * 4);//µÁ—πB
	Disp.Voltage[2] = (uint32_t)((double)((ComBuf.rec.buf[7]<<8) + ComBuf.rec.buf[8]) * 4);//µÁ—πC
...
//	Disp.Power =((ComBuf.rec.buf[31]<<8) + ComBuf.rec.buf[32])*25;//10A
	Disp.Current[0] = ((ComBuf.rec.buf[9]<<8) + ComBuf.rec.buf[10])*2;//µÁ¡˜A
	Disp.Current[1] = ((ComBuf.rec.buf[11]<<8) + ComBuf.rec.buf[12])*2;//µÁ¡˜B
	Disp.Current[2] = ((ComBuf.rec.buf[13]<<8) + ComBuf.rec.buf[14])*2;//µÁ¡˜C
	
	Disp.Pa[0] = ((ComBuf.rec.buf[15]<<8) + ComBuf.rec.buf[16])*80;//”–π¶π¶¬ A
	Disp.Pa[1] = ((ComBuf.rec.buf[17]<<8) + ComBuf.rec.buf[18])*80;//”–π¶π¶¬ B
	Disp.Pa[2] = ((ComBuf.rec.buf[19]<<8) + ComBuf.rec.buf[20])*80;//”–π¶π¶¬ C
	
	Disp.Pf[0] = ((ComBuf.rec.buf[21])<<8) + (ComBuf.rec.buf[22]);//π¶¬ “Ú ˝A
	Disp.Pf[1] = ((ComBuf.rec.buf[23])<<8) + (ComBuf.rec.buf[24]);//π¶¬ “Ú ˝B
	Disp.Pf[2] = ((ComBuf.rec.buf[25])<<8) + (ComBuf.rec.buf[26]);//π¶¬ “Ú ˝C
